...
\draw[-stealth, blue, thick](0,0)--(3,2)node[below right]{$\bf{x_1}$};
\draw[-stealth, blue, thick](0,0)--(3,-1)node[above right]{$\bf{x_2}$};
\draw[-,dashed](6,-2)node[right]{$L_2$}--(-6,2);
\draw[-,dashed](-6,-4)--(6,4)node[right]{$L_1$};
\node[above] at (0,2){$c_1>0,\,c_2<0$};
\node[right] at (2,0.3){$c_1>0,\,c_2>0$};
\node[below] at (0,-2){$c_1<0,\,c_2>0$};
\node[left] at (-2,-0.3){$c_1<0,\,c_2<0$};
\end{tikzpicture}
\end{center}
