...
\begin{solution}
    $$h(t)= \frac{1}{2}\int_0^t \tau \sin 2(t-\tau)\,d\tau$$
\end{solution}
...
\begin{solution}
    $$h(t)= \int_0^t e^{-2\tau} \cos 3(t-\tau)\,d\tau$$
\end{solution}
...
\begin{solution}
   $\frac{s}{s^2+4} \leftrightarrow \cos 2t$ and $\frac{1}{s^2+9}\leftrightarrow \frac{1}{3}\sin 3t$, so $h(t)= \frac{1}{3}\int_0^t \sin 3\tau\cos 2(t-\tau)\,d\tau$
...
\begin{solution}
    $$h(t) = \int_0^t e^{-\tau}e^{-(t-\tau)} \sin (t-\tau)\,d\tau=e^{-\tau}\int_0^t \sin (t-\tau)\,d\tau$$
\end{solution}
...
\begin{solution}
    $$h(t) = \int_0^t (\tau e^{-\tau})\left(e^{-2(t-\tau)}\sin (t-\tau)\right)\, d\tau=e^{-2t}\int_0^t \tau e^\tau \sin (t-\tau)\, d\tau$$
\end{solution}
